...
generate_scatterplots_per_species <- function(data, chromosome) {
 ...
 scatterplot1<-ggplot(df, aes(x = gc, y = spacergc, color=species)) +
    geom_point(position = position_jitter(width = 0.01),
               pch = 19,
               size = 1) +
    scale_color_manual(values = c("#006E81","#FB2D32","#C3000E","#FD8A1A","#F9CF44","#0E205E"),
                     name = "Species") +
...
ggsave("composite_plot1.png", composite_plot1)
@
...
composite_plot2 <- grid.arrange(A,B,G,H,I,J, ncol = 2)
ggsave("composite_plot2.png", composite_plot2)

composite_plot3 <- grid.arrange(C,D,E,F,plot.new(),plot.new(), ncol = 2)
ggsave("composite_plot3.png", composite_plot3)
