...
# swap_cigar_indels--
#	Swap insertions for deletions (and vice versa). This corresponds to a swap
#	of the roles of target and query.
#
# (cigarInfo is as for cigar_to_string)

def swap_cigar_indels(cigarInfo):
	if (hasattr(cigarInfo,"operations")):
		operations = list(cigarInfo.operations)
	else:
		operations = list(cigarInfo)

	for (ix,(rpt,op)) in enumerate(operations):
		if   (op == "I"): operations[ix] = (rpt,"D")
		elif (op == "D"): operations[ix] = (rpt,"I")
